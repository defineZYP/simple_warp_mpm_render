...
def sample_nerd_cube(
    ...
    min_x, min_y, min_z, max_x, max_y, max_z = nerd_bound
    index = 0
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
...
def init_nerd_cube(
    ...
    for i_idx in range(num_instances):
        ...
        if (
            ...
            if 'nerd_center' in instance_param and 'nerd_cube_param' in instance_param:
                nerd_center = instance_param['nerd_center']
                nerd_cube_param = instance_param['nerd_cube_param']
                nerd_bound = [
                    nerd_center[0] - nerd_cube_param[0] / 2,
                    nerd_center[1] - nerd_cube_param[2] / 2,
                    nerd_center[1] - nerd_cube_param[2] / 2,
                    nerd_center[0] + nerd_cube_param[0] / 2,
                    nerd_center[1] + nerd_cube_param[2] / 2,
                    nerd_center[1] + nerd_cube_param[2] / 2,
    ...
    # 第二轮
    for i_idx in range(num_instances):
        _position_vec, true_num_particles = sample_nerd_cube(
            centers[i_idx],
            cube_params[i_idx],
            cube_deltas[i_idx],
            nerd_bounds[i_idx],
